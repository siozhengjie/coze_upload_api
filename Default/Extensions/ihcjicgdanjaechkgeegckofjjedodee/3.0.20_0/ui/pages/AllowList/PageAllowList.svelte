<script lang="ts">
    ...
    const tableHeaderPopupItems: MenuPopupItem[] = [
        {
            label: translateSimpleText("allowListRemoveOption"), //"Remove all websites",
            onClick: () => {
    ...
    $: tableEntryPopupItems = exclusions.map((exclusion, idx) => {
        return {
            label: translateSimpleText("deleteWebsiteModalOption"), // "Delete website",
            onClick: () => {
                websiteToDel = idx;
    ...
    async function removeAllClicked() {
        ...
        addSuccessToast(
            translateSimpleText("removeAllAllowedWebsitesSnackbar"),
            2000
        );
        loadAllowList();
    }
    ...
    async function removeItemClicked(idx: number) {
        try {
            ...
            addSuccessToast(
                translateSimpleText("allowWebsiteDeletedSnackbar"), // "The website was removed from the Allow List",
                3000
            );
    ...
    async function onWebsiteAdded(
        ...
        addInfoToast(translateSimpleText("allowWebsiteAddedSnackbar"), 2000);
        exclusions = [
            ...exclusions,
            {
...
>
    ...
    >
        {translateSimpleText("allowListSummarySafari")}
    ...
    <div class="flex flex-row w-full justify-end">
        <PrimaryButton class="gap-2" on:click={addItemClicked}>
            ...
            <span>
                {translateSimpleText("addWebsiteButton")}
                <!-- Add a website -->
            </span>
        </PrimaryButton>
    </div>
            ...
            <Table class="w-full" divClass="relative overflow-x-auto h-[330px]">
                ...
                >
                    {#if !searchMode}
                        ...
                        >
                            {translateSimpleText("urlColumnAllowList")}
                            <!-- URL / IP address -->
                        </TableHeadCell>
                        <TableHeadCell
                            style="font-weight: 500 !important"
                        ...
                        >
                            <div class="flex flex-row w-full justify-between">
                                <span class="min-w-[150px]">
                                    <!-- Allow -->
                                    {translateSimpleText("allowColumnAllowList")}
                                </span>
                    ...
                    {#each filteredExclusions as exclusion, idx}
                        ...
                        >
                            <TableBodyCell
                                ...
                                style="padding-left: 16px !important; max-width: 200px !important; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"
...
<ConfirmModal
    ...
    title={translateSimpleText("deleteAllModalTitle")}
    message={translateSimpleText("deleteAllConfirmation")}
    confirmLabel={translateSimpleText("yesDeleteButton")}
    onConfirm={removeAllClicked}
/>
<ConfirmModal
    bind:open={showConfirmDeleteModal}
    title={translateSimpleText("deleteWebsiteModalTitle")}
    message={translateSimpleText("deleteWebsiteModalConfirmation")}
    confirmLabel={translateSimpleText("yesDeleteButton")}
    onConfirm={() => {
