...
import {
    SETTING_GTLD,
    SETTING_SAFARI_UPDATER_ENABLED,
...
const MSG_TOGGLE_UPDATER = "MSG_TOGGLE_UPDATER";
...
export class SafariProtectionService implements ProtectionService {
    ...
    toggleIndividualProtectionBySettingKey(
        ...
        if (settingKey === SETTING_SAFARI_UPDATER_ENABLED) {
            return this.toggleSafariUpdater(enabled)
        }
                ...
                (response) => {
                    ...
                    return resolve(
                        new Map([
                            ...
                            [SETTING_SAFARI_UPDATER_ENABLED, updaterEnabled]
                        ])
                    );
                }
            );
    ...
    private toggleSafariUpdater(enabled: boolean): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            chrome.runtime.sendNativeMessage(
                "com.malwarebytes.browserguard",
                {action: MSG_TOGGLE_UPDATER, payload: enabled},
                (response) => {
                    if (chrome.runtime.lastError) {
                        reject(chrome.runtime.lastError);
                        return;
                    ...
                    console.debug("TOGGLE_UPDATER: Received response from safari", {
                        response: JSON.parse(response),
                    });
                    resolve(response);
                }
