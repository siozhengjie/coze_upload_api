...
import { sentryInit, sentryToggle } from "@/ui/sentry";
import * as Sentry from "@sentry/browser";

async function init() {
    initDatabases();

    await getSettingsService().setSetting(SETTING_VERBOSE_LOGGING, true);
    await initTelemetry();

    const verboseLogging = await getSettingsService().getSetting(SETTING_VERBOSE_LOGGING);
...
async function initTelemetry() {
    ...
    let telemetryEnabled = await settingsService.getSetting(SETTING_TELEMETRY_CONSENT)
    ...
    if (telemetryEnabled) {
        ...
        console.debug('INT: Sentry initialized');
    } else {
        console.debug('INT: Sentry disabled');
    }
}

async function monitorSettingsChanges() {
    getSettingsService().addOnChangeListener<boolean|undefined>(SETTING_TELEMETRY_CONSENT, (newValue) => {
        console.debug("MON: Setting changed: ", {
            key: SETTING_TELEMETRY_CONSENT,
            newValue
        });
        if (newValue === true) {
            sentryToggle(true);
            console.debug('MON: Sentry initialized');
        } else {
            sentryToggle(false);
            console.debug('MON: Sentry disabled');
        }

        // TODO: send a message to the native app to toggle sentry when we figure out how to do that
    })
}
    ...
    (
        ...
        switch (message.type) {
            ...
            case MSG_SETTINGS_SET:
                msgSettingsSet(request, sendResponse);
                return true;
            case MSG_RESET:
                msgReset();
                return true;
...
async function msgTelemetryPhishingTunnel(
    ...
    const sendTelemetry = await getSettingsService().getSetting(
        SETTING_TELEMETRY_CONSENT
    );
...
async function msgSettingsSet(
    request: BackgroundMessageRequest<SetSettingRequest>,
    sendResponse: (
        response?: BackgroundMessageResponse<boolean>
    ) => void
) {
    const res = await getSettingsService().setSetting(request.payload!.key, request.payload!.value);
